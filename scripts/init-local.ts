...
async function tablesExist() {
  ...
  return (
    tables.TableNames?.includes('sc-accounts') && tables.TableNames?.includes('sc-repositories')
  );
}
...
async function loadConvertedData() {
  ...
  try {
    ...
    try {
      const { Items: savedRepos, Count: repoCount } = await docClient.send(
        new QueryCommand({
          TableName: 'sc-repositories',
          IndexName: 'PublicRepositoriesIndex',
      ...
      const { Items: savedAccounts, Count: accountCount } = await docClient.send(
        new QueryCommand({
          TableName: 'sc-accounts',
          IndexName: 'AccountTypeIndex',
