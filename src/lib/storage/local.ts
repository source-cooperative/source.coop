...
export class LocalStorageClient implements StorageClient {
  ...
  private async getObjectMetadata(params: {
    account_id: string;
    repository_id: string;
    object_path: string;
  }): Promise<any> {
  ...
  async listObjects(params: ListObjectsParams): Promise<ListObjectsResult> {
    ...
    try {
      ...
      const objects = filteredFiles.map(
        file =>
          ({
            id: file.path,
            repository_id: params.repository_id,
      ...
      const commonPrefixes = objects.filter(obj => obj.type === 'directory').map(obj => obj.path);
