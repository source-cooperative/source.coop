...
export function useAccountRepositoryList({ account_id, next, limit }) {
	const { data, error, isLoading } = useSWR(
		{
			path: !account_id ? null : `/repositories/${account_id}`,
			args: { next, limit },
		},
		{ refreshInterval: 0 }
	);
...
export function useRepositoryList({ next, limit, tags, q }) {
	const { data, error, isLoading } = useSWR(
		{ path: "/repositories/", args: { next, limit, tags, q } },
		{ refreshInterval: 0 }
	);
	...
	var { data, error, isLoading } = useSWR(
		{
			path:
				!account_id || !repository_id
					? null
					: `/repositories/${account_id}/${repository_id}/services`,
		},
		{ refreshInterval: 0 }
	);
