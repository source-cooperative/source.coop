...
// Analytics tabs (maintainers/owners/admins). The query param is rewritten
// by middleware to the internal /-/analytics route, so object paths can
// never be shadowed and layouts (which can't read search params) stay out
// of the loop.
export const productAnalyticsUrl = (account_id: string, product_id: string) =>
  `/${account_id}/${product_id}?tab=analytics`;
export const accountAnalyticsUrl = (account_id: string) =>
  `/${account_id}?tab=analytics`;
...
export const editServiceAccountUrl = (account_id: string, service_account_id: string) =>
  `/edit/account/${account_id}/service-accounts/${service_account_id}`;
