import { Box, Alert, Grid } from "theme-ui";
...
export default function AuthForm(input: AuthFormProps) {
    var {children, flow, onSubmit, group, gridColumns, gridColumnsTemplate, disabled} = input;
      ...
      <Box
        ...
        onSubmit={handleSubmit}>
        {
          flow.ui.nodes.map((node) => {
            ...
            if (node.group == "default" || (group && node.group == group)) {
                return (
                    <FormInput
                      sx={{gridColumnStart: 1, gridColumnEnd: "end"}}
                      name={node.attributes.name}
                      required={node.attributes.required}
                      message={node.messages.length > 0 ? node.messages[0] : null}
        ...
        <Grid sx={{
          gridTemplateColumns: gridColumnsTemplate ? gridColumnsTemplate : ["1fr"],
          gridGap: 4,
        }}>
        { flow.ui.messages && flow.ui.messages.length > 0 ? <Alert variant={flow.ui.messages[0].type} sx={{gridColumnStart: 1, gridColumnEnd: "end"}}>{flow.ui.messages[0].text}</Alert> : <></>}
        {
          flow.ui.nodes.map((node) => {
            if (node.attributes.type == "hidden") {
              return <></>
            ...
            var gridColumnEnd = "end";
            if (gridColumns && gridColumns[node.attributes.name]) {
                gridColumnStart = gridColumns[node.attributes.name]["start"];
                gridColumnEnd = gridColumns[node.attributes.name]["end"];
            }

            if (node.type == "input" && node.attributes.name == "traits.country") {
              return (
                <FormSelect
                  sx={{gridColumnStart: gridColumnStart, gridColumnEnd: gridColumnEnd}}
                  name={node.attributes.name}
                  required={node.attributes.required}
            ...
            if (node.type == "input" && node.attributes.type == "text") {
              if (node.meta.label.text == "ID") {
                return (
                  <FormInput
                    ...
                    name={node.attributes.name}
                    required={node.attributes.required}
                    label={node.meta.label.text == "ID" ? "E-Mail" : node.meta.label.text}
                    message={node.messages.length > 0 ? node.messages[0] : null}
                    defaultValue={node.attributes.value}
                    disabled={node.attributes.disabled || disabled}
                    type="email"
            ...
            if (node.type == "input" && node.attributes.type == "password") {
              return (
                <FormInput
                  sx={{gridColumnStart: gridColumnStart, gridColumnEnd: gridColumnEnd}}
                  name={node.attributes.name}
            ...
            if (node.type == "input" && node.attributes.type == "email") {
                return (
                  <FormInput
                    sx={{gridColumnStart: gridColumnStart, gridColumnEnd: gridColumnEnd}}
                    name={node.attributes.name}
                    required={node.attributes.required}
                    label={node.meta.label.text == "ID" ? "Email" : node.meta.label.text}
                    message={node.messages.length > 0 ? node.messages[0] : null}
                    defaultValue={node.attributes.value}
                    disabled={node.attributes.disabled || disabled}
                    type="email"
...
AuthForm.defaultProps = defaultAuthFormProps;