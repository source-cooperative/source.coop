...
import { AdminBreakdownChart, seriesColor } from "@/components/features/analytics";
...
/**
 * URL params → view state, restricted to the offered dimensions. Exported
 * for tests: a scoped view leaks other accounts' traffic if a dropped
 * dimension can be filtered or grouped back in from the query string.
 */
export function parseState(
  ...
  // Scoped views drop dimensions (an account explorer has no "account"
  // axis); params naming one are ignored rather than silently widening.
  const offered = new Set<string>(dimensions);
  const defaultGroupBy = dimensions.filter((d) => d === "product");
  return {
      ...
      groupByParam === undefined
        ...
        : [
            ...new Set(
              groupByParam
                .split(",")
                .filter((d): d is AdminDimension => offered.has(d)),
            ),
          ],
...
interface BreakdownExplorerProps {
  /**
   * Page the controls link back to. Any query it carries (the `?tab=analytics`
   * the middleware rewrites) is preserved — a GET form's action drops its own
   * query string, so those params are re-emitted as hidden inputs.
   */
  baseUrl: string;
  searchParams: Record<string, string | string[] | undefined>;
  /** Dimensions offered as filters and group-by chips, in display order */
  dimensions: AdminDimension[];
  /**
   * Filters applied to every query but hidden from the UI and URLs — how a
...
/**
 * The traffic explorer: range/filter controls, a stacked-series chart, and a
 * ranked totals table over data-proxy request analytics. Admin sees every
 * dimension; a scoped view pins one via `scopeFilters` and drops it from
 * `dimensions` so it can't be filtered or grouped away.
 */
export async function BreakdownExplorer({
  baseUrl,
  searchParams,
  dimensions,
  scopeFilters,
  ...
  try {
    breakdown = await getAdminBreakdown({
      ...state,
      filters: { ...state.filters, ...scopeFilters },
    });
  } catch (error) {
  ...
  const dimensionChips = dimensions.map((dim) => ({
    key: dim,
    label: ADMIN_DIMENSIONS[dim].label,
  }));

  const base = new URL(baseUrl, "http://relative.invalid");
  const baseParams = Object.fromEntries(base.searchParams);

  const pageUrl = (next: PageState): string => {
    const params = new URLSearchParams(baseParams);
    params.set("groupBy", next.groupBy.join(","));
    if (next.from) params.set("from", next.from);
    if (next.to) params.set("to", next.to);
    if (next.bucketMinutes) params.set("interval", String(next.bucketMinutes));
    if (next.metric === "bytes") params.set("metric", next.metric);
    for (const [dim, value] of Object.entries(next.filters)) {
        ...
        <Flex gap="5" wrap="wrap">
          ...
          >
            ...
            <AdminFiltersForm
              ...
              action={base.pathname}
              dimensions={dimensionChips}
              defaults={{
