...
async function migrateData() {
  // Migrate accounts
  const accounts = await docClient.send(
    new ScanCommand({
  ...
  const repositories = await docClient.send(
    new ScanCommand({
...
async function validateMigration() {
  // Validate accounts
  const oldAccounts = await docClient.send(
    new ScanCommand({
      TableName: 'sc-accounts',
    })
  );

  const newAccounts = await docClient.send(
    new ScanCommand({
      TableName: 'Accounts_v2',
    })
  );
  ...
  if (oldAccounts.Items.length !== newAccounts.Items.length) {
    throw new Error(
      `Account count mismatch: ${oldAccounts.Items.length} vs ${newAccounts.Items.length}`
    );
  }
  ...
  const oldRepos = await docClient.send(
    new ScanCommand({
      TableName: 'sc-repositories',
    })
  );

  const newRepos = await docClient.send(
    new ScanCommand({
      TableName: 'Repositories_v2',
    })
  );
  ...
  if (oldRepos.Items.length !== newRepos.Items.length) {
    throw new Error(
      `Repository count mismatch: ${oldRepos.Items.length} vs ${newRepos.Items.length}`
    );
  }
